<div class="mb-4 p-3 bg-light rounded shadow-sm">
        ...
        <select id="tienda-selector" class="form-select">
            @foreach($storePickupData['tiendas'] as $tienda)
                <option 
                    ...
                    data-telefono="{{ $tienda->telefono }}"
                    data-horario-semana="{{ $tienda->horario_semana }}"
    ...
    <div class="row mt-4">
        <div class="col-md-6">
            <strong>Dirección:</strong> <span class="store-address">{{ $storePickupData['tiendas']->first()->direccion }}</span><br>
            <strong>Teléfono:</strong> <span class="store-phone">{{ $storePickupData['tiendas']->first()->telefono }}</span><br>
            ...
            <iframe src="{{ $storePickupData['tiendas']->first()->google_maps_url }}" width="540" height="360" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
        </div>
        <div class="col-md-6">
            <!-- Calendario interactivo -->
            <div class="mb-4 p-3 bg-light rounded shadow-sm">
                <h5 class="mb-3 text-primary"><i class="bi bi-calendar"></i> Seleccione la fecha y hora para recoger su pedido</h5>
            ...
            <div id="payment-section" class="p-4 bg-light rounded shadow-sm mt-4" style="display: none;">
                <div class="row align-items-center">
                    ...
                    <div class="col-12 col-md-6 text-md-start text-center">
                        <form id="storePickupForm" action="{{ route('storepickup.save') }}" method="POST">
                            ...
                            <button type="submit" class="btn btn-primary btn-lg d-flex justify-content-center align-items-center w-50 mx-auto">
                ...
                <div class="mt-4 text-center">
                    <p class="mb-3">No hay cobro de envío en este método de entrega. 
                        <a href="#" data-bs-toggle="modal" data-bs-target="#enviosModal">Más información acerca de esto</a>
                    </p>
                    <img src="{{ asset('storage/img/formasdepago/form_tarjeta.png') }}" alt="Formas de Pago" class="img-fluid" style="max-width: 150px;">
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    pickupDateInput.addEventListener('input', function() {
        ...
        // Bloquear la selección del mismo día o día siguiente
        const currentDate = new Date();
        const minDate = new Date(currentDate);
        minDate.setDate(currentDate.getDate() + 1);

        if (selectedDate <= minDate) {
            dateErrorElement.textContent = 'No puedes programar un pedido para días anteriores, el mismo día o el día siguiente.';
            dateErrorElement.style.display = 'block';
            this.value = '';

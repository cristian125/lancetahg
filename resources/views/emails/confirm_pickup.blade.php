...
<body>
    <div class="container">
        ...
        <p>Hola,</p>
        <p>Tu pedido #{{ $order->id }} ha sido recibido y está siendo procesado. Nos comunicaremos contigo para confirmar cuándo podrás recogerlo en la tienda seleccionada.</p>

        <div class="order-details">
            <h4>Detalles del Pedido</h4>
            <p><strong>ID de Pedido:</strong> {{ $order->id }}</p>
            <p><strong>Total con IVA:</strong> ${{ number_format($order->total_con_iva, 2) }} MXN</p>
            <p><strong>Fecha de Recogida (estimada):</strong> {{ $pickupDate }}</p>
            <p><strong>Hora de Recogida (estimada):</strong> {{ $pickupTime }}</p>
        </div>
